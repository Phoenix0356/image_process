...
def net_output(w_in2hidden, w_hidden2out, input_signals, return_hidden):
    inputs = npy.array(input_signals, ndmin=2).T
    hidden_in = npy.dot(w_in2hidden, inputs)
    hidden_out = exp(hidden_in)
    final_in = npy.dot(w_hidden2out, hidden_out)
    final_out = exp(final_in)
...
def net_train(target_list, input_signals, w_in2hidden, w_hidden2out, learn_speed):
    targets = npy.array(target_list, ndmin=2).T
    inputs = npy.array(input_signals, ndmin=2).T
    final_out, hidden_out = net_output(w_in2hidden, w_hidden2out, input_signals, 1)
...
def plot_image(pixels: npy.array):
    pyt.imshow(pixels.reshape((28, 28)), cmap='gray')
    pyt.show()
